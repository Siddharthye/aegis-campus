...
const WANTED: readonly WantedGap[] = [
  {
    ...
    minimumViable:
      "A webhook that POSTs { text, from, receivedAt } to a URL we configure.",
  },
  {
    ...
    minimumViable:
      "Any endpoint that emits { cameraId, hazard, confidence, at } over SSE or a webhook.",
  },
  {
    ...
    minimumViable:
      "POST { message, zone } that plays audio, plus a delivery receipt.",
  },
  {
    ...
    minimumViable:
      "Audio in, { transcript, language, category? } out. Accuracy matters more than latency.",
  },
  {
    ...
    minimumViable:
      "A bridge that turns a button press into POST /api/sentinel/arm with a location.",
  },
  {
    ...
    minimumViable:
      "GET route(from, to) → { distanceM, etaSeconds, path[] }. Offline routing preferred.",
  },
];
...
export default function WantedPage() {
  return (
    <OpsShell
      title="Six things we will buy"
      subtitle="Real gaps in AEGIS, each with a mounting point already rendered in the product."
    >
      <div className="mx-auto max-w-3xl">
        <p className="mt-4 max-w-2xl text-[15px] leading-relaxed text-ops-muted">
          These are real gaps in AEGIS that we deliberately did not build. Each
          one already has a mounting point rendered in the running product, so
          integrating it is one entry in{" "}
          <code className="font-mono text-[13px] text-ops-faint">
            integrations.config.ts
          </code>{" "}
          and one environment variable — not an afternoon of glue.
        </p>
        <p className="mt-3 max-w-2xl text-[13px] leading-relaxed text-ops-faint">
          We buy on three criteria, in order: it runs from a fresh clone in
          front of us, it has a README, and it needs no key we do not already
          have. Bring it running.
        </p>
          ...
          {WANTED.map((gap, index) => (
            ...
            >
              <div className="flex flex-wrap items-baseline gap-3">
                ...
                <h2 className="text-[16px] font-semibold text-ops-text">
                  {gap.title}
                </h2>
              ...
              <p className="mt-2.5 text-[13px] leading-relaxed text-ops-muted">
                {gap.why}
              </p>
              ...
              <dl className="mt-3 grid gap-2 border-t border-ops-border pt-3 sm:grid-cols-2">
                <div>
                  ...
                  <dd className="mt-0.5 font-mono text-[11px] text-ops-muted">
                    {gap.mountsAt}
                  </dd>
                </div>
                <div>
                  <dt className="ops-label text-ops-faint">
        ...
        <section className="mt-10 rounded-lg border border-ops-accent/30 bg-ops-accent/5 p-5">
          ...
          <p className="mt-2 text-[13px] leading-relaxed text-ops-muted">
            SIREN (geofenced alerts), ATLAS (live incident map + triage engine),
            and FUSION (duplicate report fusion). Each ships a REST + SSE API,
            an iframe widget, a React component, and a framework-free JS client
            — so any stack integrates in about ten minutes.
          </p>
          <Link
            href="/"
